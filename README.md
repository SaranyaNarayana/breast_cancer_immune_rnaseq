...
├── .gitignore
└── README.md
└── Saranya_Narayana_BC_immune_heterogenity.pptx
```
...
|-----------|---------|-------------|
| R version | 4.4.0 | 4.5.2 (developed on) |
| RAM | 16 GB | 32 GB |
...
**Bioconductor:**
```
TCGAbiolinks, SummarizedExperiment, DESeq2, edgeR, limma,
GSVA, GSEABase, msigdbr, biomaRt, fgsea, ConsensusClusterPlus
```

**CRAN:**
```
dplyr, tidyr, tibble, ggplot2, patchwork, cowplot, ggpubr,
ggrepel, pheatmap, RColorBrewer, umap, factoextra, cluster,
immunedeconv
```

All versions are pinned in `renv.lock`. Run `setup_renv.R` to restore them exactly.

---

## Reproducibility

- Package versions are locked via `renv.lock` — `setup_renv.R` restores the exact environment.
- `set.seed()` is used in all scripts involving stochasticity (clustering, UMAP).
- Each script writes a timestamped log to `logs/` for full traceability.
- Large intermediate files (`.rds`) are git-ignored and regenerated by the pipeline from scratch.
- Raw TCGA-BRCA data is downloaded programmatically by script 01 from the public [GDC portal](https://portal.gdc.cancer.gov/projects/TCGA-BRCA) — no manual download required.
...
**TCGA download timeout:**
```r
options(timeout = 600)  # increase to 10 minutes
```

**edgeR / DGEList not found:**
```r
library(edgeR)  # must be loaded explicitly before use
```
...
1. Parker JS et al. (2009) *J Clin Oncol* 27:1160–1167 — PAM50 classification
2. Perou CM et al. (2000) *Nature* 406:747–752 — Molecular subtypes
3. TCGA Network (2012) *Nature* 490:61–70 — BRCA comprehensive analysis
4. Hänzelmann S et al. (2013) *BMC Bioinformatics* 14:7 — GSVA
5. Finotello F et al. (2019) *Genome Medicine* 11:34 — quanTIseq
6. Newman AM et al. (2015) *Nature Methods* 12:453–457 — CIBERSORT
7. Racle J et al. (2017) *eLife* 6:e26476 — EPIC
8. Schmid P et al. (2018) *NEJM* 379:2108–2121 — Immunotherapy in breast cancer
9. Denkert C et al. (2018) *Lancet Oncol* 19:40–50 — TILs and outcomes

---

## License

This project is licensed under the MIT License. TCGA data is publicly available and subject to the [TCGA Data Use Agreement](https://www.cancer.gov/about-nci/organization/ccg/research/structural-genomics/tcga/using-tcga/policies).
